...
MODELS = {
    ...
    "Gemma-3-12B": {
        ...
        # Availability may depend on the Gemini API account/region.
...
# Prompts
# ======================================================
# Minimal harmonised prompt used for the Google-hosted benchmark models
SYSTEM_MSG = (
...
for MODEL_NAME, MODEL_CFG in MODELS.items():
    print(f"\nRunning {MODEL_NAME} (VOTES=1)...")
    print(f"Runtime config: workers={MAX_WORKERS}, initial_rps={INITIAL_RPS}, min_rps={MIN_RPS}, max_rps={MAX_RPS}")
    ...
    USE_STRUCTURED_OUTPUT = bool(MODEL_CFG.get("structured_output", False))
    ...
    def _post_once(text: str, system_msg: str, temperature: float, sample_i=None):
        ...
        # Gemini 2.5 Flash supports schema-constrained JSON and thinking control.
        # Gemma uses the same target format through prompting and downstream validation.
    ...
    def call_model(text: str, temperature: float, sample_i=None, force_system_msg: Optional[str] = None):
        ...
        for attempt in range(1, MAX_RETRIES + 1):
            ...
            try:
                short_text = PROMPT_BASE.replace("{TEXT}", text)
                r = _post_once(short_text, system_msg=system_msg, temperature=temperature, sample_i=sample_i)
                ...
                if not raw_text.strip():
                    ...
                    # إذا وصل MAX_TOKENS جرّب مرة ثانية بنفس النص
                    if "MAX_TOKENS" in fr:
                ...
                last_err = f"exception__{repr(e)[:500]}"
                _sleep_backoff(attempt, salt)
                continue
    ...
    if err_t == 1:
        ...
        raise RuntimeError(f"Test call failed on {MODEL_NAME}. Review the API error above before running the full dataset.")
    else:
    ...
    summary_rows.append({
        ...
        "run_id": RUN_ID,
        "setup": ("zero-shot + single inference (V1) — Google-hosted model — "
                  f"structured_output={USE_STRUCTURED_OUTPUT} — "
                  f"thinking_budget={MODEL_CFG.get('thinking_budget')} — "
                  "parse/invalid-label retries + empty-candidate handling"),
        "votes": VOTES,
        "temp_votes": TEMP_VOTES,
        "max_workers": MAX_WORKERS,
