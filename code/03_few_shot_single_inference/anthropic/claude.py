...
# ===============================
# CONFIG — CLAUDE FEW-SHOT, SINGLE INFERENCE (V1)
# ===============================
VOTES = 1
TEMP_VOTES = 0.0  # best for stability
...
# Claude-Sonnet-4.6 is the only Anthropic model reported in the manuscript.
MODELS = {
...
RUN_ID = datetime.now().strftime("%Y%m%d_%H%M%S") + "__FEWSHOT_V1"
...
# 4) FEW-SHOT PROMPT
#   if unsure -> neutral
#   examples should be SHORT and CLEAN
# ======================================================
SYSTEM_MSG = (
...
Sentence: "اليوم فرحت كثير لما نجحت"
Output: {"label":"joy"}

Sentence: "انا معصب جدا من هالتصرف"
Output: {"label":"anger"}

Sentence: "حاسه بضيق وحزن من اللي صار"
Output: {"label":"sadness"}

Sentence: "خفت كثير من هالخبر"
Output: {"label":"fear"}

Sentence: "الجو اليوم عادي وما عندي شيء جديد"
...
If the emotion is unclear, mixed, factual, or ambiguous, return:
{"label":"neutral"}
...
def build_prompt(text: str) -> str:
    return PROMPT.replace("{FEW_SHOT_EXAMPLES}", FEW_SHOT_EXAMPLES).replace("{TEXT}", text)
...
for MODEL_NAME, MODEL_ID in MODELS.items():
    print(f"\nRunning {MODEL_NAME} (FEW-SHOT, VOTES=1, TEMP=0)...")
    ...
    save_confusion_matrix(
        ...
        STRICT_LABELS,
        title=f"{MODEL_NAME} Confusion Matrix (incl. unknown) — FEW-SHOT V1",
    ...
    save_confusion_matrix(
        cm_5,
        ALLOWED_LABELS,
        title=f"{MODEL_NAME} Confusion Matrix (5 labels) — FEW-SHOT V1",
    ...
    summary_rows.append({
        ...
        "run_id": RUN_ID,
        "setup": "few-shot + single inference (V1)",
        "votes": VOTES,
        "temp_votes": TEMP_VOTES,
        "max_workers": MAX_WORKERS,
...
summary_df.to_csv(
    os.path.join(METRIC_DIR, f"{PROVIDER_NAME}__RUN{RUN_ID}__SUMMARY_FEWSHOT_V1_strict_json.csv"),
    index=False,
    encoding="utf-8-sig",
)

print("\nClaude few-shot benchmark completed successfully (single inference, T=0).")
print("RUN_ID:", RUN_ID)
