...
# ===============================
# CONFIG (VOTE=1, TEMP=0, FEW-SHOT, STRICT JSON UNKNOWN)
# ===============================
VOTES = 1
TEMP_VOTES = 0.0
...
RUN_ID = datetime.now().strftime("%Y%m%d_%H%M%S") + "__V1_FEWSHOT"
...
# FEW-SHOT EXAMPLES
# IMPORTANT:
# - keep fixed across all models
# - balanced: one example per label
# - ideally outside the test set
# ======================================================
FEW_SHOTS = [
...
def build_few_shot_messages(text: str):
    ...
    for ex in FEW_SHOTS:
        messages.append({"role": "user", "content": f'Sentence: "{ex["text"]}"'})
        messages.append({"role": "assistant", "content": json.dumps({"label": ex["label"]}, ensure_ascii=False)})
...
# ======================================================
def extract_mistral_content(resp) -> str:
    try:
...
# ======================================================
# CALL MISTRAL
# Mistral docs support chat.complete(messages=...)
# and response_format for JSON mode / schema mode. :contentReference[oaicite:1]{index=1}
# ======================================================
def call_model(model_id, model_name, text, temperature, limiter: AdaptiveRateLimiter, sample_i=None, vote_i=None):
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            ...
            raw = extract_mistral_content(resp)
            if raw:
            ...
            last_err = f"status_{status}" if status is not None else (str(e)[:200] or "exception")
...
def classify_one(model_id, model_name, text, limiter: AdaptiveRateLimiter, sample_i=None):
    ...
    if err_flag == 1:
        ...
        return pred, 1, abstained, calls_made, vote_pattern, unknown_votes, reason
    ...
    if pred == UNKNOWN_LABEL:
        ...
        vote_pattern = "0"
        reason = parse_reason
        return pred, 0, abstained, calls_made, vote_pattern, unknown_votes, reason
...
for MODEL_NAME, MODEL_ID in MODELS.items():
    ...
    with ThreadPoolExecutor(max_workers=MAX_WORKERS) as executor:
        ...
        for fut in tqdm(as_completed(futures), total=n, desc=MODEL_NAME):
            i = futures[fut]
            pred, err_calls, abst_inc, calls_made, pat, unk_votes, reason = fut.result()
    ...
    save_confusion_matrix(
        cm_with_unknown,
        STRICT_LABELS,
        title=f"{MODEL_NAME} Confusion Matrix (incl. unknown) — V1 Few-shot",
    ...
    save_confusion_matrix(
        cm_5,
        ALLOWED_LABELS,
        title=f"{MODEL_NAME} Confusion Matrix (5 labels) — V1 Few-shot",
    ...
    if STORE_VOTES_LIST:
        out_pred_df["votes_list"] = [
            json.dumps([prediction], ensure_ascii=False)
            for prediction in y_pred
        ]
        output_columns = UNIFIED_PRED_COLS + ["votes_list"]
    else:
        output_columns = UNIFIED_PRED_COLS

    out_pred_df = out_pred_df[output_columns]
    out_pred_df.to_csv(
    ...
    summary_rows.append({
        ...
        "model_id": MODEL_ID,
        "run_id": RUN_ID,
        "setup": "few-shot + single inference (V1) + Mistral JSON mode",
        "votes": VOTES,
        "temp_votes": TEMP_VOTES,
        "max_workers": MAX_WORKERS,
...
summary_df.to_csv(
    os.path.join(METRIC_DIR, f"{PROVIDER_NAME}__RUN{RUN_ID}__SUMMARY_V1_fewshot_strict_json.csv"),
    index=False,
    encoding="utf-8-sig",
)

print("\nMistral few-shot benchmark completed successfully for all manuscript models (VOTE=1, TEMP=0).")
print("RUN_ID:", RUN_ID)
